function clusters = cluster_table_wani_hongji(clusters, varargin)
...
%
% Also you need canlab atlas_labels_combined data in your path. If you don't 
% have this, an error will happen in the following line. 
%     load atlas_labels_combined_info;
%
% ----------------------------------------------------------
% Example
...
% ----------------------------------------------------------
% ----------------------------------------------------------
% So generic example of the function is 
%    cluster_table_wani(cl, 1, 0, [1 32], 'writefile', savename);
% Here, 1 is dosubcluster; 0 is nolabel, but this will do label anyways;
% [1 32] is degree of freedom to calculate T values. 
% ----------------------------------------------------------
%
...
%
% Edited 10/1/12 We are not using Carmack labeling any more. Wani added
% alternative labeling system, which is canlab's atlas_localization_tools
% and Automated anatomical labeling (AAL). 
...
if length(varargin) > 2, df = varargin{3}; end
...
if isfield(clusters, 'center') && exist('M') == 1 && isfield(clusters, 'from_cluster')
    ...
    % wani
    fprintf(fid, 'x\ty\tz\tdir\tT\tZ\tneg_logp\tvoxels\tvolume_mm3\t');
...
%     load(fullfile(atlas_basedir, 'ROI_MNI_V4_List.mat'));
    ...
    for i = 1:size(cmatx, 1)
        ...
        if cmatx(i, 3)>0, act_deact = 'pos';
        elseif cmatx(i, 3)<0, act_deact = 'neg'; end
        
        cl_num = 0;
        for j = 1:length(cl)
            XYZmm_in_cl = (cl(j).XYZmm')';
            for voxel_num_in_cl = 1:length(XYZmm_in_cl)
                if sum(XYZmm == XYZmm_in_cl(:,voxel_num_in_cl)) == 3, cl_num = j; break, end
            end
            if cl_num ~= 0, break, end
        end
        
        if cl_num == 0
            clusters(i).shorttitle2 = 'outside';
        else
        ...
        % wani changed orders... x/y/z, T, Z, -logp, voxnum, voxsize(mm3)
        fprintf(fid, '%3.0f\t%3.0f\t%3.0f\t%s\t%3.2f\t%3.2f\t%3.2f\t%4.0f\t%4.0f\t', x(i), y(i), z(i), act_deact, spm_u(10^-abs(cmatx(i, 3)), df, 'T'), spm_u(10^-abs(cmatx(i, 3)), df, 'Z'), abs(cmatx(i, 3)), cmatx(i, 2), cmatx(i, 2).*prod(clusters(i).voxSize)); 
...
%         end
        ...
        if dosubcluster
            ...
            if length(whsc)>1
                for j = whsc
                    if ~dolabs
                        print_row(subc(j), j, clusters, fid, df)
                    else
                        print_row(subc(j), j, clusters, fid, df, stricbm2)
                    end
                end
            end
        end
...
function print_row(clusters, i, bigcl, fid, df, varargin)
...
% ========= Wani's additional setting
atlas_basedir = '/Users/hongji/Dropbox/NeuroImaging/spm/batch_aal'; %'/Users/clinpsywoo/Dropbox/MATLAB/3aal_for_spm8';
% atlas_basedir = '/Volumes/RAID1/canlab/wani/Documents/MATLAB/3aal_for_spm8';
atlas = fullfile(atlas_basedir, 'ROI_MNI_V4.nii');
MNID = spm_vol(atlas);
load(fullfile(atlas_basedir, 'ROI_MNI_V4_Border.mat'));
...
if cl_num == 0
    region_title2 = 'outside';
else    
    region_title2 = names{cl_num};
end
% =========
