...
cd('/Users/hongji/Dropbox/IRRH_2019S/scripts_git/analysis/imaging/first_level/model4/ttest');
...
for beta_i = 1:size(beta_fnames,1)
    ...
    if colors_type(subj_idx-2,beta_i) == 1 % blue
        if stim_type(subj_idx-2,beta_i) == 0 % base
            blue_base = [blue_base, dat.dat];
        elseif stim_type(subj_idx-2,beta_i) == 1 % warm
...
save('XX_rb_sub04_trial.mat', 'rb')
save('XX_rp_sub04_trial.mat', 'rp')
save('XX_rw_sub04_trial.mat', 'rw')
save('XX_bb_sub04_trial.mat', 'bb')
save('XX_bp_sub04_trial.mat', 'bp')
save('XX_bw_sub04_trial.mat', 'bw')


%% new_pain
new_base.dat = [mean(new_base.dat(:,1:3),2), mean(new_base.dat(:,4:6),2), mean(new_base.dat(:,7:9),2), ...
    mean(new_base.dat(:,10:12),2), mean(new_base.dat(:,13:15),2), mean(new_base.dat(:,16:18),2), ...
    mean(new_base.dat(:,19:21),2), mean(new_base.dat(:,22:24),2), mean(new_base.dat(:,25:27),2), ...
    mean(new_base.dat(:,28:30),2)];
...
save('bb_run5_sub03.mat', 'bb_run5_sub03');
...
save('bw_run5_sub03.mat', 'bw_run5_sub03');
